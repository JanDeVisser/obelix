...
ErrorOr<void> int_unary_expression(Image& image, Obelix::Assembler::Segment& code, UnaryExpression const& expr)
{
    ...
    case TokenCode::Minus: {
        if (expr.operand()->type()->type()->type() == ObelixType::TypeUnsigned)
...
ErrorOr<void> byte_unary_expression(Image& image, Obelix::Assembler::Segment& code, UnaryExpression const& expr)
{
    ...
    switch (expr.op().code()) {
    case TokenCode::Minus: {
        if (expr.operand()->type()->type()->type() == ObelixType::TypeChar)
            return Error { ErrorCode::SyntaxError, "Cannot negate unsigned numbers" };
...
ErrorOrNode output_jv80_processor(std::shared_ptr<SyntaxNode> const& tree, OutputJV80Context& ctx)
{
    ...
    case SyntaxNodeType::BinaryExpression: {
        ...
        if (expr->lhs()->type()->type()->type() == ObelixType::TypeUnknown) {
            return Error { ErrorCode::UntypedExpression, expr->lhs()->to_string(0) };
        }
        if (expr->rhs()->type()->type()->type() == ObelixType::TypeUnknown) {
            return Error { ErrorCode::UntypedExpression, expr->rhs()->to_string(0) };
        }

        if ((expr->lhs()->type()->type()->type() == ObelixType::TypeInt && expr->lhs()->type()->type()->type() == ObelixType::TypeInt) || (expr->lhs()->type()->type()->type() == ObelixType::TypeUnsigned && expr->lhs()->type()->type()->type() == ObelixType::TypeUnsigned)) {
            TRY_RETURN(int_int_binary_expression(ctx.image(), *code, *expr));
        }
        if ((expr->lhs()->type()->type()->type() == ObelixType::TypeByte && expr->lhs()->type()->type()->type() == ObelixType::TypeByte) || (expr->lhs()->type()->type()->type() == ObelixType::TypeChar && expr->lhs()->type()->type()->type() == ObelixType::TypeChar)) {
            TRY_RETURN(byte_byte_binary_expression(ctx.image(), *code, *expr));
        }
        if (expr->lhs()->type()->type()->type() == ObelixType::TypeBoolean && expr->lhs()->type()->type()->type() == ObelixType::TypeBoolean) {
    ...
    case SyntaxNodeType::UnaryExpression: {
        ...
        if (expr->operand()->type()->type()->type() == ObelixType::TypeUnknown) {
            return Error { ErrorCode::UntypedExpression, expr->operand()->to_string(0) };
        }

        if (expr->operand()->type()->type()->type() == ObelixType::TypeInt || expr->operand()->type()->type()->type() == ObelixType::TypeUnsigned) {
            TRY_RETURN(int_unary_expression(ctx.image(), *code, *expr));
        }
        if (expr->operand()->type()->type()->type() == ObelixType::TypeByte || expr->operand()->type()->type()->type() == ObelixType::TypeChar) {
            TRY_RETURN(byte_unary_expression(ctx.image(), *code, *expr));
        }
        if (expr->operand()->type()->type()->type() == ObelixType::TypeBoolean) {
