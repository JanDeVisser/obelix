...
void _user_init(void) {
  if (User < 1) {
    ...
    if ((_getpwnam_bufsize = (int) sysconf(_SC_GETPW_R_SIZE_MAX)) < 0) {
...
data_t * _user_new_getpwnam(user_t *user, va_list args) {
  ...
  if (!err) {
    ...
    user->_d.str = (user->fullname && *(user->fullname)) ? user->fullname : user->name;
    data_set_string_semantics(user, StrSemanticsExternStatic);
  } else {
